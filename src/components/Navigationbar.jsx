...
const Navigationbar=()=> {
  ...
  const renderNavDropDownItem =()=>{
    ...
    <NavDropdown.Item key={ category.id } onClick={()=>{ handleOnClickCategory(category.id) }}>{ category.name }</NavDropdown.Item>
    )
  }
  ...
  return (
    ...
    <Navbar.Collapse id="responsive-navbar-nav">
      <Nav className="ml-auto pr-md-5">
        ...
        { user? <NavLink to ="/" onClick={()=>{ handleOnclickLogout() }} className ={styles.link }>Logga ut / {user.userName}</NavLink>:<NavLink className ={ styles.link } to="/users/login" >Logga in</NavLink> }
      </Nav>
    </Navbar.Collapse>
  </Navbar>
  )
